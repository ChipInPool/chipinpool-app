...
export async function registerRoutes(
  ...
  // ==========================================================================
  // STRIPE CONNECT V2 MARKETPLACE ROUTES
  // ==========================================================================
  ...
  // Create a Stripe Connect V2 connected account for a merchant
  app.post("/api/connect/accounts", requireAuth, async (req, res, next) => {
    try {
      const userId = req.session.userId!;
      const { displayName, contactEmail } = z.object({
      ...
      // Create V2 connected account with platform responsible for pricing and fees
      ...
      const [existingMerchant] = await db.select().from(merchants).where(eq(merchants.userId, userId));
      
      if (existingMerchant) {
        // Update existing merchant with Connect ID
      ...
      console.error('[Connect] Create account error:', error.message);
      next(error);
    }
  });

  // Get connected account status (always fetch from API for demo)
  app.get("/api/connect/accounts/:accountId/status", requireAuth, async (req, res, next) => {
    try {
      const { accountId } = req.params;
      const stripe = await getUncachableStripeClient();

      // Fetch account with includes for status check
      ...
      console.error('[Connect] Get account status error:', error.message);
      next(error);
    }
  });

  // Create account link for onboarding
  app.post("/api/connect/accounts/:accountId/onboarding-link", requireAuth, async (req, res, next) => {
    try {
      const { accountId } = req.params;
      const stripe = await getUncachableStripeClient();
      ...
      const accountLink = await (stripe as any).v2.core.accountLinks.create({
        ...
        use_case: {
          ...
          account_onboarding: {
            ...
            refresh_url: `${appUrl}/marketplace/onboarding?accountId=${accountId}`,
            return_url: `${appUrl}/marketplace/onboarding?accountId=${accountId}&onboarding=complete`,
          },
        },
      });
      ...
      console.error('[Connect] Create account link error:', error.message);
      next(error);
    }
  });

  // Get current user's merchant/connect account
  app.get("/api/connect/my-account", requireAuth, async (req, res, next) => {
    try {
      const userId = req.session.userId!;
      ...
      const stripe = await getUncachableStripeClient();
      try {
        const account = await (stripe as any).v2.core.accounts.retrieve(merchant.stripeConnectId, {
          include: ["configuration.recipient", "requirements"],
        });
        ...
        res.json({
          hasAccount: true,
          accountId: merchant.stripeConnectId,
          merchantId: merchant.id,
          companyName: merchant.companyName,
          readyToReceivePayments,
          onboardingComplete,
  ...
  // Create a product at platform level with merchant mapping
  app.post("/api/connect/products", requireAuth, async (req, res, next) => {
    try {
      const userId = req.session.userId!;
      const { name, description, priceInCents, currency, imageUrl } = z.object({
        name: z.string().min(1),
        description: z.string().optional(),
      ...
      const [merchant] = await db.select().from(merchants).where(eq(merchants.userId, userId));
      if (!merchant || !merchant.stripeConnectId) {
        return res.status(400).json({ error: 'You need to create a connected account first' });
      }
      ...
      const { platformProducts } = await import('@shared/schema');
      const [newProduct] = await db.insert(platformProducts).values({
        merchantId: merchant.id,
      ...
      console.error('[Connect] Create product error:', error.message);
      next(error);
    }
  });

  // Get all products for storefront
  app.get("/api/connect/storefront", async (req, res, next) => {
    try {
      const { platformProducts } = await import('@shared/schema');
      
      // Get all active products with their merchants
      const products = await db.select({
        ...
        merchant: {
          ...
          logo: merchants.logo,
          stripeConnectId: merchants.stripeConnectId,
        },
      })
      .from(platformProducts)
      .innerJoin(merchants, eq(platformProducts.merchantId, merchants.id))
      .where(eq(platformProducts.isActive, true))
      .orderBy(desc(platformProducts.createdAt));
  ...
  app.get("/api/connect/my-products", requireAuth, async (req, res, next) => {
    try {
      const userId = req.session.userId!;
      const [merchant] = await db.select().from(merchants).where(eq(merchants.userId, userId));
      ...
      const products = await db.select()
        .from(platformProducts)
        .where(eq(platformProducts.merchantId, merchant.id))
        .orderBy(desc(platformProducts.createdAt));
  ...
  app.post("/api/connect/checkout", async (req, res, next) => {
    try {
      ...
      if (!merchant.stripeConnectId) {
        return res.status(400).json({ error: 'Merchant not properly configured' });
      }
      ...
      console.error('[Connect] Checkout error:', error.message);
      next(error);
    }
  });

  // V2 Account webhook handler for thin events
  app.post("/api/connect/webhooks", express.raw({ type: 'application/json' }), async (req, res) => {
    const sig = req.headers['stripe-signature'];
    const webhookSecret = process.env.STRIPE_CONNECT_WEBHOOK_SECRET;

    if (!webhookSecret) {
      console.log('[Connect Webhook] No webhook secret configured');
    ...
    try {
      ...
      // Parse thin event
      const thinEvent = (stripe as any).parseThinEvent(req.body, sig, webhookSecret);
      
      console.log(`[Connect Webhook] Received thin event: ${thinEvent.type}`);
      ...
      switch (event.type) {
        case 'v2.core.account[requirements].updated':
          console.log(`[Connect Webhook] Requirements updated for account: ${event.data?.object?.id}`);
          // Handle requirements update - could notify merchant
          break;

        case 'v2.core.account[.recipient].capability_status_updated':
          console.log(`[Connect Webhook] Capability status updated for account: ${event.data?.object?.id}`);
          // Handle capability status change
          break;
