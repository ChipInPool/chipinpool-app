...
export default function Home() {
  ...
  const completedPools = MOCK_POOLS.filter(p => p.status === 'completed');
    ...
    <Layout>
      ...
      <section className="relative overflow-hidden rounded-3xl border border-white/5 bg-card/30 mb-12">
        ...
        <div className="relative z-10 px-8 py-16 md:py-24 max-w-2xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-semibold mb-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <span>The Social Way to Pay</span>
          </div>
          ...
          <div className="flex gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
            <Link href="/create">
              <Button size="lg" className="font-semibold text-base px-8 h-12 shadow-lg shadow-primary/20 hover:scale-105 transition-transform">
                Start a Pool
              </Button>
            </Link>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {/* Add New Placeholder */}
          <Link href="/create">
            <div className="h-full min-h-[300px] rounded-xl border border-dashed border-white/10 bg-white/[0.02] hover:bg-white/[0.05] hover:border-primary/30 transition-all flex flex-col items-center justify-center gap-4 cursor-pointer group">
